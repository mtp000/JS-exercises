...
function removeFromString(str, startInd, numChar) {
    let newStr = str.split('');
    newStr.splice(startInd, numChar);
    return newStr.join('');
}
...
function includes(collection, value, startInd) {
    if (Array.isArray(collection) || typeof collection === 'string') {
        let startIndex = startInd || 0;
        for (let i = startInd; i < collection.length; i++) {
